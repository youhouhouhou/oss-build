...
./mvnw ${MAVEN_SETTINGS} help:effective-pom | grep 'Downloading:' | awk '!(NR%10)'
./mvnw ${MAVEN_SETTINGS} help:effective-pom > ${EFFECTIVE_POM_FILE}
...
maven_analysis() {
    if [[ "$(basename $(pwd))" == *-config ]] && ([ -f "application.yml" ] || [ -f "application.properties" ]); then
        ...
        ./mvnw ${MAVEN_SETTINGS} -U clean package | grep -v 'Downloading:' | grep -Ev '^Generating .+\.html\.\.\.'
    else
        echo "maven_analysis sonar"
        ./mvnw ${MAVEN_SETTINGS} sonar:sonar
    fi
}
...
maven_test_and_build() {
    ...
    if [ "true" == "${BUILD_PUBLISH_DEPLOY_SEGREGATION}" ]; then
        ...
        ./mvnw ${MAVEN_SETTINGS} -U clean org.apache.maven.plugins:maven-antrun-plugin:run@clean-local-deploy-dir deploy | grep -v 'Downloading:' | grep -Ev '^Generating .+\.html\.\.\.'
    else
        maven_pull_base_images
        # reduce log avoid travis 4MB limit
        ./mvnw ${MAVEN_SETTINGS} -U clean install | grep -v 'Downloading:' | grep -Ev '^Generating .+\.html\.\.\.'
    fi
}
...
maven_publish_snapshot() {
    ...
    ./mvnw ${MAVEN_SETTINGS} help:active-profiles
    if [ "true" == "${BUILD_PUBLISH_DEPLOY_SEGREGATION}" ]; then
        export MAVEN_OPTS="${MAVEN_OPTS} -Dwagon.source.filepath=${DEPLOY_LOCAL_REPO_IF_NEED} -Dactive_publish_segregation=true"
        export MAVEN_OPTS="${MAVEN_OPTS} -DaltDeploymentRepository=repo::default::file://${DEPLOY_LOCAL_REPO_IF_NEED}"
        ...
        ./mvnw ${MAVEN_SETTINGS} org.codehaus.mojo:wagon-maven-plugin:merge-maven-repos@deploy-merge-maven-repos docker:build docker:push
    else
        ./mvnw ${MAVEN_SETTINGS} deploy
    fi
}
...
maven_publish_release() {
    ...
    if [ "true" == "${BUILD_PUBLISH_DEPLOY_SEGREGATION}" ]; then
        ...
        export MAVEN_OPTS="${MAVEN_OPTS} -DaltDeploymentRepository=repo::default::file://${DEPLOY_LOCAL_REPO_IF_NEED}"
        export MAVEN_OPTS="${MAVEN_OPTS} -Dbuild.publish.channel=${BUILD_PUBLISH_CHANNEL}"
        ./mvnw ${MAVEN_SETTINGS} org.codehaus.mojo:wagon-maven-plugin:merge-maven-repos@deploy-merge-maven-repos docker:removeImage docker:build docker:push
    else
        ./mvnw ${MAVEN_SETTINGS} deploy
    fi
}
...
maven_deploy_site() {
    ...
    if [ "true" == "${BUILD_SITE}" ]; then
        ...
        if [ "${INFRASTRUCTURE}" != "github" ]; then
            echo yes | ./mvnw ${MAVEN_SETTINGS} site:site site:stage site:stage-deploy | grep -v 'Downloading:' | grep -Ev '^Generating .+\.html\.\.\.'
        else
            # -X enable debug logging for Maven to avoid build timeout (not generate output)
            ./mvnw ${MAVEN_SETTINGS} site site-deploy | grep -v 'Downloading:' | grep -Ev '^Generating .+\.html\.\.\.'
        fi
    fi
}
